            ...
            for (int c = 0; c < shrunkenTensor.c; ++c) {
                for (int h = 0; h < newH; ++h) {
                    for (int w = 0; w < newW; ++w) {
                        auto sliced = shrunkenTensor.tensor.chip(n, 0).chip(c, 0)
                                                    .slice(Eigen::array<int, 2>({h * stride, w * stride}), Eigen::array<int, 2>({pooling, pooling}));
                        Eigen::array<int, 2> reduction_dims = {0, 1};
                        auto reduced = sliced.reduce(reduction_dims, Eigen::internal::MaxReducer<float>()).eval();
                        result.tensor(n, c, h, w) = float(reduced);
                    }
                }
            }
        }
